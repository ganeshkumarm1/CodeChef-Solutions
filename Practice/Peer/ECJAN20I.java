...
class AMChain {
    ...
    public Point[] findLowerHull() {
        ...
        for(int i = totalPoints - 1; i >= 0; i --) {
            ...
            // We do <= comparison to  remove collinear points
            // If we want to include collinear points we can change it to <=
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (t -- > 0) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                points[i] = new Point(Integer.parseInt(row[0]), Integer.parseInt(row[1]));
            }

            AMChain amChain = new AMChain(points);

            Point[] result = amChain.findBoundingPoints();
